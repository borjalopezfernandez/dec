...
private
   ...
   def writeFixedHeader(bIsEmergencyMode=false)
      ...
      if @bIsOutgoing == true then
         ...
         @theFile.puts("      <File_Description>List of Unknown files present in #{@entity}</File_Description>")
      end
   ...
   def writeDataBlock(arrHashData)
      ...
      @theFile.puts(%Q{    <List_of_Unknown_Files>})
   ...
   def writeElement(arrFiles)
	   arrFiles.each{|file|
		   @theFile.puts(%Q{        <Unknown_File>#{File.basename(file)}</Unknown_File>})
		}
   end
   #-------------------------------------------------------------
