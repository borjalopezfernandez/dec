...
include CUC::DirUtils
  ...
  if (system(command)==0) then
      smsLog= %Q{#{ENV['RPFBIN']}/write2Log.bin --o #{File.basename($0)} --m "Move files from #{@directory} to #{@_rejectDirectory}"  --info --fileTx}
      system(smsLog)
   else
      smsLog= %Q{#{ENV['RPFBIN']}/write2Log.bin --o #{File.basename($0)} --m "Can not move files from #{@directory} to #{@_rejectDirectory}"  --error --fileTx}
      system(smsLog)
   end
...
def createLog(entity)
   # initialize logger
   loggerFactory = CUC::Log4rLoggerFactory.new("moveFilesToRejectDirectory:#{entity}", "#{ENV['DEC_CONFIG']}/dec_log_config.xml")
   if @isDebugMode then
