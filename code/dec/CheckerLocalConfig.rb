...
class CheckerLocalConfig
   ...
   def initialize(localServerStruct, strInterfaceCaption = "", logger = nil)
      @logger        = logger
      @isDebugMode   = false
   ...
   def checkLocalConfig(b4Receive, b4Send)
      ...
      ###  
         ...
         if retVal == false then
            @logger.error("[DEC_712] I/F #{@entity}: Cannot reach local dir #{@localElement[:uploadDir]}")
            ret = false
         end
      ...
      ###
         ...
         retVal = checkLocalDirectory(dir, true)
         if retVal == false then
            @logger.error("[DEC_712] I/F #{@entity}: Cannot reach local dir #{@localElement[:uploadTemp]}")         
            ret = false
         end
...
#         end
      ...
      if b4Receive then
         ...
         arrDownDirs.each{|dirStruct|
            ...
            if retVal == false then
               @logger.error("[DEC_612] I/F #{@entity}: Cannot reach local dir #{dirStruct[:directory]}")
               bWarning = true
            else
               bNoError = true
            end
